...
export const ControlledCopiesView: React.FC<ControlledCopiesViewProps> = ({ viewType: propViewType = "all" }) => {
  ...
  const handleReportLostDamagedConfirm = (type: "Lost" | "Damaged") => {
    ...
    if (selectedCopyForReportLostDamaged) {
      navigate(`${ROUTES.DOCUMENTS.CONTROLLED_COPIES.DESTROY(selectedCopyForReportLostDamaged.id)}?type=${type}`);
    }
  };
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full h-9 pl-10 pr-4 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 transition-colors"
                />
              </div>
            </div>
            <div>
