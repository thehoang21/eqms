...
export const DocumentTypesTab: React.FC = () => {
  ...
  }, [mockData, searchQuery, statusFilter, modifiedFromDate, modifiedToDate]);
              ...
              {filteredItems.length > 0 ? (
                paginatedItems.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="py-2 px-2 sm:py-3.5 sm:px-4 text-xs sm:text-sm whitespace-nowrap">
                      <span className="inline-flex items-center px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-lg text-[10px] sm:text-xs font-medium bg-emerald-50 text-emerald-700 border border-emerald-200">
                        {item.shortCode}
                      </span>
                    </td>
                    <td className="py-2 px-2 sm:py-3.5 sm:px-4 text-xs sm:text-sm whitespace-nowrap">
        ...
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl animate-in zoom-in-95 fade-in duration-200">
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
            <h2 className="text-lg font-semibold text-slate-900">
              {isEdit ? "Edit" : "Add New"} Document Type
            </h2>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4">
            ...
            <div>
              ...
              />
              <p className="text-xs text-slate-500 mt-1">
                The next document will be numbered: {formData.shortCode}.
                {String(formData.currentSequence + 1).padStart(4, "0")}.01
              </p>
            </div>
