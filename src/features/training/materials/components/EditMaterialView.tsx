...
export const EditMaterialView: React.FC = () => {
  ...
  if (!existingData || existingData.status !== "Draft") {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] space-y-4">
        ...
        <h2 className="text-lg font-semibold text-slate-900">Cannot Edit Material</h2>
        <p className="text-sm text-slate-500 text-center max-w-sm">
          Only materials with <span className="font-semibold text-slate-700">Draft</span> status can be edited.
  ...
  return <EditMaterialForm materialId={materialId!} existingData={existingData} />;
};
...
const EditMaterialForm: React.FC<EditMaterialFormProps> = ({ materialId, existingData }) => {
  const navigate = useNavigate();
  ...
  const [uploadMode, setUploadMode] = useState<UploadMode>(existingData.uploadMode);
  ...
  return (
    <div className="space-y-6 w-full flex-1 flex flex-col">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-3 lg:gap-4">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 text-slate-500 mt-1 text-xs whitespace-nowrap overflow-x-auto">
            ...
            <span className="sm:hidden">...</span>
            <span className="text-slate-400 mx-1">/</span>
            <span className="text-slate-700 font-medium">Edit Training Material</span>
          </div>
        </div>
      ...
      <div className="flex items-start gap-3 px-4 py-3 bg-amber-50 border border-amber-200 rounded-xl">
        ...
        <div>
          <p className="text-sm font-medium text-amber-800">Editing Draft Material</p>
          <p className="text-xs text-amber-700 mt-0.5">
            You are editing <span className="font-semibold">{formData.materialCode} — {formData.materialName}</span>.
            Edits are only permitted while the material is in <span className="font-semibold">Draft</span> status.
            Save your changes or submit for review when ready.
          </p>
        </div>
      </div>
                ...
                >
                  <div
                    className={cn(
                      ...
                      isCurrent ? "bg-emerald-600" : isCompleted ? "bg-emerald-100" : "bg-slate-100"
                    )}
                    style={{
                      clipPath: isFirst
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="p-5">
              ...
              <div className="flex items-center gap-1 p-1 bg-slate-100 rounded-lg mb-4">
                <button
                  onClick={() => { setUploadMode("file"); setNewFile(null); setIsReplacingFile(false); }}
                  className={cn(
                ...
                <button
                  onClick={() => { setUploadMode("link"); setNewFile(null); setIsReplacingFile(false); }}
                  className={cn(
                    "flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-xs font-medium transition-all",
                    uploadMode === "link" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"
                  )}
                >
                ...
                <>
                  ...
                  {!isReplacingFile && !newFile && (
                    <div className="border border-emerald-200 rounded-xl p-4 bg-emerald-50/40">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-slate-500 mt-0.5">
                            ...
                            {getFileTypeFromName(existingData.existingFile.name)}
                          </p>
                      ...
                      <button
                        onClick={() => { setIsReplacingFile(true); setTimeout(() => fileInputRef.current?.click(), 50); }}
                  ...
                  {isReplacingFile && !newFile && (
                    ...
                    >
                      ...
                      <button
                        onClick={(e) => { e.stopPropagation(); setIsReplacingFile(false); }}
                  ...
                  {newFile && (
                    <div className="space-y-3">
                      <div className="border border-slate-200 rounded-xl p-4 bg-slate-50/50">
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-slate-500 mt-0.5">
                              {formatFileSize(newFile.size)} · {getFileTypeFromName(newFile.name)}
                            </p>
                      ...
                      <p className="text-xs text-slate-500">
                        Replacing:{" "}
                        <span className="font-medium text-slate-700">{existingData.existingFile.name}</span>
                        {" "}→{" "}
        ...
        <div className="xl:col-span-7">
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-200">
              ...
              <p className="text-xs text-slate-500 mt-0.5">Update the details for this training material</p>
            </div>
            <div className="p-5 space-y-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-1">Material code cannot be changed after creation.</p>
                </div>
                <div>
                  <label className="text-xs sm:text-sm font-medium text-slate-700 mb-1.5 block">Version</label>
                  <div className="relative">
                    <input
                      type="text"
                  ...
                  <p className="text-xs text-slate-400 mt-1">Version is managed by the approval workflow.</p>
                </div>
              </div>
