...
const SOP_ACTIONS = [
  { icon: FileText, label: "View Document" },
...
const InfoTile: React.FC<{
  ...
  <div className="bg-white border border-slate-200 rounded-lg p-3 shadow-sm">
    ...
    <p className={cn("text-sm font-semibold text-slate-800 truncate", mono && "font-mono")}>
      {value}
    </p>
  </div>
);
...
export const HeaderActionDrawer: React.FC<HeaderActionDrawerProps> = ({
  ...
  const employeeStats = useMemo(() => {
    ...
    const overdue   = required.filter((sop) => getCell(emp.id, sop.id)?.status === "Overdue").length;
    const expiring  = required.filter((sop) => getCell(emp.id, sop.id)?.status === "ExpiringSoon").length;
    const pending   = required.filter((sop) => getCell(emp.id, sop.id)?.status === "Pending").length;
    const total     = required.length;
    const rate      = total > 0 ? Math.round((qualified / total) * 100) : 100;
    return { qualified, overdue, expiring, pending, total, rate };
  }, [type, data]);
  ...
  const sopStats = useMemo(() => {
    ...
    const overdue   = required.filter((emp) => getCell(emp.id, sop.id)?.status === "Overdue").length;
    const expiring  = required.filter((emp) => getCell(emp.id, sop.id)?.status === "ExpiringSoon").length;
    const pending   = required.filter((emp) => getCell(emp.id, sop.id)?.status === "Pending").length;
    const total     = required.length;
    const rate      = total > 0 ? Math.round((qualified / total) * 100) : 100;
    return { qualified, overdue, expiring, pending, total, rate };
  }, [type, data]);
      ...
      >
        ...
        <div className="px-4 py-3 border-b border-slate-100 shrink-0 bg-white">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2.5 flex-1 min-w-0">
              {empData ? (
                <>
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      <span className="text-[10px] font-mono font-semibold bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded-lg">
                ...
                <>
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Training Material</span>
                      <span className="text-[10px] font-mono font-semibold bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded-lg">
                        {sopData.code}
                      </span>
                      <span
                        className={cn(
                          ...
                          CATEGORY_COLORS[sopData.category] ?? "bg-slate-50 text-slate-600 border-slate-200"
                        )}
                      >
                        {sopData.category}
                      </span>
                    ...
                    <p className="text-[11px] text-slate-400 font-mono">{sopData.version}</p>
                  </div>
                </>
              ) : null}
            </div>
            ...
            <button
              onClick={handleClose}
              className="p-1.5 hover:bg-slate-100 rounded-full transition-colors text-slate-400 hover:text-slate-600 flex-shrink-0"
              aria-label="Close"
            >
              <X className="h-4 w-4" />
            </button>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/40">
          ...
          {empData ? (
            ...
            <section className="grid grid-cols-2 gap-3">
              ...
              <InfoTile icon={FileText}  label="Document Code"  value={sopData.code} mono />
            </section>
          ) : null}
          ...
          {stats && (
            <section className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
              ...
              <div className="grid grid-cols-4 gap-2">
                ...
                <StatTile value={stats.overdue}   label="Overdue"   color="text-red-600"     bg="bg-red-50" />
                <StatTile value={stats.expiring}  label="Expiring"  color="text-amber-600"   bg="bg-amber-50" />
                <StatTile value={stats.pending}   label="Pending"   color="text-blue-600"    bg="bg-blue-50" />
              </div>
            </section>
          )}
