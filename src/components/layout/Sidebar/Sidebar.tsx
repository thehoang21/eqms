...
export const Sidebar: React.FC<SidebarProps> = ({
    ...
    return (
      <div key={item.id} className="w-full relative">
        ...
        <button
          ...
          className={cn(
            "w-full flex items-center group relative transition-colors duration-150 ease-out overflow-visible z-10 outline-none focus-visible:ring-2 focus-visible:ring-primary/20",
            "py-2.5 md:py-3",
        ...
        >
          {level === 0 && isActive && (
            <div className="absolute left-0 top-0 bottom-0 w-1 bg-primary transition-all duration-200 ease-out"></div>
          )}

          <div className={cn(
            "flex items-center justify-center transition-transform duration-150 ease-out relative",
          ...
          >
            {Icon && (
              <Icon className={cn(
                "shrink-0 transition-colors duration-150 ease-out h-5 w-5",
            ...
            {isCollapsed && (
              <div className="absolute left-full ml-2 px-3 py-1.5 bg-slate-800 text-white text-xs md:text-sm rounded opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-50 shadow-lg transition-opacity duration-150 ease-out delay-75 font-medium">
                {item.label}
          ...
          {hasChildren && (
            <div
              ...
              className={cn(
                "rounded-md hover:bg-slate-200 transition-all duration-150 ease-out ml-auto flex items-center justify-center h-6 w-6",
  ...
  return (
    <>
      ...
      {isMobileOpen && (
        <div
          className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-40 lg:hidden transition-opacity duration-200 ease-out"
          style={{
      ...
      <aside
        className={cn(
          ...
          // Mobile: Off-canvas with slide animation
