...
export const FormModal: React.FC<FormModalProps> = ({
  ...
  return createPortal(
    ...
    >
      ...
      >
        <div className="px-4 sm:px-6 pt-4 sm:pt-5 pb-3 sm:pb-4 border-b border-slate-100 bg-white shrink-0">
          <div className="flex items-start justify-between gap-4">
            ...
            <button
              onClick={onClose}
              aria-label="Close"
              className="text-slate-400 hover:text-slate-600 transition-colors -mt-1 -mr-2 p-2 rounded-lg hover:bg-slate-100 min-w-[44px] min-h-[44px] flex items-center justify-center"
            >
              <X className="h-5 w-5" />
            </button>
          </div>
          {description && (
            <div className="mt-2 sm:mt-3 text-xs sm:text-sm text-slate-500 leading-relaxed">{description}</div>
          )}
        </div>
